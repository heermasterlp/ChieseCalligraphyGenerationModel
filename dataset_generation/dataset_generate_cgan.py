...
def pickle_dataset(dataset, dataset_dir, src_font, dst_font, image_mode, canvas_size, x_offset, y_offset, flag="train"):
    """
    ...
    for i in range(len(dataset)):
        ...
        dst_img = draw_single_char(char, dst_font, image_mode, canvas_size, x_offset, y_offset)
        src_img = draw_single_char(char, src_font, image_mode, canvas_size, x_offset, y_offset)

        img = Image.new(image_mode, (canvas_size * 2, canvas_size), default_color)
        img.paste(dst_img, (0, 0))
        img.paste(src_img, (canvas_size, 0))
...
parser.add_argument('--src_font', dest='src_font', required=True, help='path of the src font')
parser.add_argument('--dst_font', dest='dst_font', required=True, help='path of the dst font')
...
if __name__ == '__main__':
    ...
    dst_font = ImageFont.truetype(args.dst_font, size=args.char_size)

    pickle_dataset(dataset=char_dataset.char_trainset, dataset_dir=args.data_dir, src_font=src_font,
                   dst_font=dst_font, image_mode=args.image_mode, canvas_size=args.canvas_size,
                   x_offset=args.x_offset, y_offset=args.y_offset, flag="train")

    # val
    pickle_dataset(dataset=char_dataset.char_valset, dataset_dir=args.data_dir, src_font=src_font,
                   dst_font=dst_font, image_mode=args.image_mode, canvas_size=args.canvas_size,
                   x_offset=args.x_offset, y_offset=args.y_offset, flag="val")

    # test
    pickle_dataset(dataset=char_dataset.char_testset, dataset_dir=args.data_dir, src_font=src_font,
                   dst_font=dst_font, image_mode=args.image_mode, canvas_size=args.canvas_size,
                   x_offset=args.x_offset, y_offset=args.y_offset, flag="test")
